    ...
    def test_strategy_gkr_configs_have_no_violations(self):
        ...
        if strategies_dir.exists():
            for gkr_file in strategies_dir.glob("*.gkr.yaml"):
                ...
                assert "full_kelly" not in content, f"{gkr_file.name}: full_kelly reference detected"
...
class TestTestRelated:
    def test_complete_go_mappings_cover_all_packages(self):
        ...
        internal_dirs = [
            f"internal/{d.name}/" for d in sorted((ROOT / "internal").iterdir())
            if d.is_dir() and not d.name.startswith("_")
            and any(d.rglob("*_test.go"))
        ]
...
class TestCriticalPaths:
    def test_critical_paths_json_is_valid(self):
        ...
        for name, cfg in data.get("paths", {}).items():
            for f in cfg.get("files", []):
                file_path = ROOT / f
                if "*" not in f:
                    assert file_path.exists(), f"Critical path '{name}' references missing file: {f}"
    ...
    def test_kill_switch_has_reentrancy_guards(self):
        ...
        found_flight = ("killSwitch" in content and "InFlight" in content) or "_killSwitchInFlight" in content
