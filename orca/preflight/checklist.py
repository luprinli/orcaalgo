...
def run_preflight_checks() -> list[CheckResult]:
    ...
    if gkr_files:
        results.append(
            CheckResult("gkr_strategies", "pass", f"Found {len(gkr_files)} GKR strategy files")
        )
    else:
    ...
    for gkr_file in gkr_files:
        try:
            ...
            if errors:
                results.append(
                    CheckResult(
                        f"validate_{gkr_file.stem}", "fail", f"{len(errors)} validation errors"
                    )
                )
            else:
                results.append(
                    CheckResult(f"validate_{gkr_file.stem}", "pass", "Strategy validated")
                )
    ...
    try:
        ...
        if ks.is_locked:
            results.append(
                CheckResult("kill_switch_guard", "fail", "Kill-switch default state is locked")
            )
        else:
            results.append(
                CheckResult(
                    "kill_switch_guard", "pass", "Kill-switch re-entrancy guard model validated"
                )
            )
    except Exception as e:
    ...
    try:
        ...
        if gkr_for_cli:
            ...
            if result.returncode == 0:
                results.append(
                    CheckResult(
                        "kill_switch_cli_smoke", "pass", "orca validate subprocess call successful"
                    )
                )
            else:
                results.append(
                    CheckResult(
                        "kill_switch_cli_smoke", "warn", f"CLI validate exited {result.returncode}"
                    )
                )
        else:
            results.append(
                CheckResult(
                    "kill_switch_cli_smoke", "warn", "No GKR file available for CLI smoke test"
                )
            )
    except Exception as e:
        results.append(
            CheckResult("kill_switch_cli_smoke", "warn", f"CLI subprocess smoke test failed: {e}")
        )
    ...
    if importlib.util.find_spec("orca.attribution.slicer"):
        results.append(
            CheckResult(
                "balance_reconcile", "pass", "PnL attribution engine available for reconciliation"
            )
        )
    else:
        results.append(
            CheckResult(
                "balance_reconcile",
                "warn",
                "Attribution engine not importable; balance reconciliation unavailable",
            )
        )
    ...
    if cal_files:
        ...
        results.append(
            CheckResult("calibration_recency", "pass", f"Latest calibration: {latest.name}")
        )
    else:
        results.append(
            CheckResult(
                "calibration_recency", "warn", "No calibration reports found; run `orca calibrate`"
            )
        )
    ...
    if pf_files:
        results.append(
            CheckResult("propfirm_profiles", "pass", f"Found {len(pf_files)} prop firm profiles")
        )
    else:
        ...
        try:
            ...
            if hash_file.exists():
                ...
                if verify_instance_hash(ir, expected):
                    results.append(
                        CheckResult(
                            f"hash_{gkr_file.stem}", "pass", f"instance hash verified: {ih[:12]}"
                        )
                    )
                else:
                    results.append(
                        CheckResult(
                            f"hash_{gkr_file.stem}",
                            "fail",
    ...
    try:
        ...
        if dq and hasattr(dq, "passed"):
            ...
            results.append(
                CheckResult("data_integrity", "warn", "Data quality validator returned no results")
            )
    except ImportError:
    ...
    for name, path in [
        (
            "account_credentials_migration",
            "internal/db/migrations/000043_account_credentials.up.sql",
        ),
