...
def validate_ir(ir: StrategyIRV04, profile: str = "research") -> list[Diagnostic]:
    ...
    if ir.ir_version != "qst-ir/0.4":
        diagnostics.append(
            Diagnostic(
                ...
                severity="error",
                phase="validation",
                message=f"Expected qst-ir/0.4, got {ir.ir_version}",
            )
        )
    ...
    if not has_core:
        diagnostics.append(
            Diagnostic(
                code="MISSING_CORE_CAPABILITY",
                severity="error",
                phase="capability_gate",
                message="Strategy must declare 'core' capability",
            )
        )
    ...
    if len(node_ids) < len(ir.strategy.nodes):
        diagnostics.append(
            Diagnostic(
                ...
                severity="error",
                phase="validation",
                message="Duplicate node IDs found in strategy",
            ),
        )
        ...
        if src_node not in node_ids:
            diagnostics.append(
                Diagnostic(
                    ...
                    severity="error",
                    phase="validation",
                    message=f"Output reference '{ref}' does not match any node",
                ),
            )
