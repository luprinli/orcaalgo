...
def brier_score(predictions: list[float], outcomes: list[int]) -> float:
    ...
    n = len(predictions)
    return sum((p - o) ** 2 for p, o in zip(predictions, outcomes, strict=False)) / n
...
def murphy_decomposition(
    ...
    for b in range(n_bins):
        ...
        if count == 0:
            bin_stats.append(
                BinStats(bin_start=low, bin_end=high, count=0, mean_prediction=0.0, hit_rate=0.0)
            )
            continue
        ...
        bin_stats.append(
            BinStats(
                bin_start=low, bin_end=high, count=count, mean_prediction=mean_p, hit_rate=hit_r
            )
        )
