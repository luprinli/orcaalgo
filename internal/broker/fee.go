package broker

// BrokerageFeeConfig is the single source of truth for commission/fee calculations
// shared by the backtest engine and paper broker (R3 — docs/backtest_live_parity_audit.md).
type BrokerageFeeConfig struct {
	PerTradeFixed float64
	PerContract   float64
	MinFee        float64
	SECTxnFee     float64
	Enabled       bool
}

// DefaultBrokerageFee returns the default retail-ish fee schedule.
func DefaultBrokerageFee() BrokerageFeeConfig {
	return BrokerageFeeConfig{
		PerTradeFixed: 0.35,
		PerContract:   0.65,
		MinFee:        1.00,
		SECTxnFee:     0.0000229,
		Enabled:       true,
	}
}

// CalculateFee computes the total commission for a trade of the given quantity
// and price. Returns 0 when fees are disabled.
func (f BrokerageFeeConfig) CalculateFee(quantity float64, price float64) float64 {
	if !f.Enabled {
		return 0
	}
	fee := f.PerTradeFixed + (quantity * f.PerContract) + (quantity * price * f.SECTxnFee)
	if fee < f.MinFee {
		return f.MinFee
	}
	return fee
}
