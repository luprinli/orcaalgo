        ...
        try {
          ...
          if (completed.length > 0 || pollCount >= 60) {
            const assembled = recent.map((r: any, i: number) => ({
              ...
              pool_sortino: r.pool_sortino ?? 0,
              pool_maxdd: r.pool_maxdd ?? 0,
              pool_return_pct: r.pool_return_pct ?? 0,
              rebalance_costs: r.rebalance_costs ?? 0,
              num_trades: r.result_json?.num_trades ?? r.result_json?.trades?.length ?? 0,
              win_rate: r.result_json?.win_rate ?? 0,
              profit_factor: r.result_json?.profit_factor ?? 0,
              num_wins: r.result_json?.num_wins ?? 0,
              num_losses: r.result_json?.num_losses ?? 0,
              monte_carlo_pass_prob: r.result_json?.monte_carlo?.pass_probability ?? 0,
              breach_count: r.result_json?.correlation_breaches?.length ?? 0,
              per_strategy_stats: r.result_json?.per_strategy_stats ?? {},
