...
export function exportMatrixResultsCSV(results: ComboResult[], filename = 'matrix_results.csv') {
  const headers = [
    ...
    'GrossReturn%', 'DataSource', 'EngineVersion',
    'MaxDDDuration', 'AdverseSelectRate', 'ZeroPnLTrades', 'ExpectedPF',
    'RewardRiskRatio', 'DailyVolatility', 'TrainPct', 'MtmMaxDD%', 'MtmSharpe',
    'MLFeatureEnabled', 'DataGenID', 'WfISSharpe', 'WfOOSSharpe',
    'WfAnchorOOSSharpe', 'WfBestParams', 'WfMultiplicityWarning',
    'FirstCandle', 'LastCandle', 'DeclaredBPD', 'EffectiveBPD',
    'Implausible', 'SignalAttempts', 'SignalsPassed', 'RegimeRejected',
    'VolHalted', 'PipelineRejected', 'MLRejected', 'FillRejected',
    'CandlesSeen', 'StrategyNil', 'ExitZeroQty', 'TradesOpened',
    'CapitalZero', 'RateLimited', 'BaseSizeZero', 'QuantityTooSmall',
    'ExposureBlocked', 'PipelineRejBreakdown',
  ]
  const rows = results.map(r => [
    ...
    r.gross_return_pct?.toFixed(2) ?? '', r.data_source ?? '', r.engine_version ?? '',
    r.max_drawdown_duration ?? '',
    r.adverse_selection_rate != null ? r.adverse_selection_rate.toFixed(4) : '',
    r.zero_pnl_trades ?? '', r.expected_pf != null ? r.expected_pf.toFixed(4) : '',
    r.reward_risk_ratio != null ? r.reward_risk_ratio.toFixed(4) : '',
    r.daily_volatility != null ? r.daily_volatility.toFixed(4) : '',
    r.train_pct != null ? r.train_pct.toFixed(2) : '',
    r.mtm_max_drawdown != null ? r.mtm_max_drawdown.toFixed(2) : '',
    r.mtm_sharpe_ratio != null ? r.mtm_sharpe_ratio.toFixed(4) : '',
    r.ml_feature_enabled ?? '',
    r.data_generation_id ?? '',
    r.wf_is_sharpe != null ? r.wf_is_sharpe.toFixed(4) : '',
    r.wf_oos_sharpe != null ? r.wf_oos_sharpe.toFixed(4) : '',
    r.wf_anchor_oos_sharpe != null ? r.wf_anchor_oos_sharpe.toFixed(4) : '',
    r.wf_best_params ?? '',
    r.wf_multiplicity_warning ?? false,
    r.first_candle_time ?? '', r.last_candle_time ?? '',
    r.declared_bars_per_day != null ? r.declared_bars_per_day.toFixed(2) : '',
    r.effective_bars_per_day != null ? r.effective_bars_per_day.toFixed(2) : '',
    r.implausible ?? false,
    r.signal_attempts ?? '', r.signals_passed ?? '', r.regime_rejected ?? '',
    r.vol_halted ?? '', r.pipeline_rejected ?? '', r.ml_rejected ?? '', r.fill_rejected ?? '',
    r.candles_seen ?? '', r.strategy_nil ?? '', r.exit_signal_zero_qty ?? '', r.trades_opened ?? '',
    r.capital_zero ?? '', r.rate_limited ?? '', r.base_size_zero ?? '', r.quantity_too_small ?? '',
    r.exposure_blocked ?? '',
    r.pipeline_rejects ? JSON.stringify(r.pipeline_rejects) : '',
  ].map(escapeCSV).join(','))
  const csv = [headers.join(','), ...rows].join('\n')
  downloadBlob(csv, filename)
}
