...
export default function SettingsPage() {
  ...
  const handleSave = async () => {
    if (!cfg) return
    setSaving(true)
    try { await settings.update(cfg); setMsg(t('settings:updated', 'Settings saved')) }
    catch (err) { setMsg(err instanceof Error ? err.message : t('settings:updateFailed', 'Save failed')) }
  ...
  if (loading) return <div className="card"><p className="text-muted">{t('settings:loading', 'Loading settings...')}</p></div>

  return (
    <div>
      <div className="flex-between mb-4"><h1 style={{ margin: 0 }}>{t('settings:title', 'Settings')}</h1></div>

      {msg && <p className="text-muted mb-4" style={{ color: msg.includes('fail') || msg.includes('Fail') ? 'var(--danger)' : 'var(--success)' }}>{msg}</p>}

      <div style={{ display: 'flex', flexDirection: 'column', gap: 16, maxWidth: 600 }}>
        <div className="card">
          <h2>{t('settings:riskParameters', 'Risk Parameters')}</h2>
        ...
        <div className="card">
          <h2>{t('settings:general', 'General')}</h2>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            <div className="flex-between">
              <label className="text-muted">{t('settings:defaultTimeframe', 'Default Timeframe')}</label>
            ...
            <div className="flex-between">
              <label className="text-muted">{t('settings:defaultCapital', 'Default Capital')}</label>
            ...
            <div className="flex-between">
              <label className="text-muted">{t('settings:dataSource', 'Data Source')}</label>
