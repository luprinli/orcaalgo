  ...
  return (
    <div>
      <div className="flex-between mb-4">
        <h1 style={{ margin: 0 }}>Calibration Audit</h1>
        <button className="btn btn-primary" onClick={runCalibration} disabled={loading}>
          {loading ? 'Running Audit...' : 'Run Calibration'}
        </button>
      </div>
      ...
      {!report && !loading && (
        <div className="card">
          <p className="text-muted">
            Run a calibration audit to evaluate probability forecast quality using Brier score decomposition,
            Platt scaling recommendations, and per-segment reliability analysis.
          </p>
        </div>
      )}
      ...
      {loading && (
        <div className="card">
          <p className="text-muted">Running calibration audit against trade ledger...</p>
        </div>
      )}
        ...
        <div>
          <div className="grid-3 mb-4">
            <div className="metric-card">
              <div className="metric-label">Overall Brier</div>
            ...
            <div className="metric-card">
              <div className="metric-label">Uncertainty</div>
            ...
            <div className="metric-card">
              ...
              <div className="metric-value" style={{ color: report.overall?.needs_calibration ? 'var(--warn)' : 'var(--success)' }}>
                {report.overall?.needs_calibration ? 'Yes' : 'No'}
              </div>
            </div>
          </div>
              ...
              {segment && (
                <div>
                  ...
                  {segment.bin_stats && segment.bin_stats.length > 0 && (
                    <div>
                      <span className="text-muted">Calibration bins (decile):</span>
          ...
          {report.generated_at && (
            <p className="text-muted">Generated: {new Date(report.generated_at).toLocaleString()}</p>
          )}
        </div>
      )}
    </div>
