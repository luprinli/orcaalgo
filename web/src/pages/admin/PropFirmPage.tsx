...
const profileSchema = z.object({
  ...
  max_daily_loss_pct: z.number({ message: 'Required' }).min(0).max(100),
  max_drawdown_pct: z.number({ message: 'Required' }).min(0).max(100),
  profit_target_pct_phase1: z.number({ message: 'Required' }).min(0).max(1000),
  profit_target_pct_phase2: z.number({ message: 'Required' }).min(0).max(1000),
  max_open_positions: z.number({ message: 'Required' }).int().min(1),
  min_trading_days: z.number({ message: 'Required' }).int().min(1),
})
...
export default function PropFirmPage() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      setMsg('Failed to load prop firm data')
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  return (
    <div>
      <div className="flex-between mb-4">
        <h1 style={{ margin: 0 }}>Prop Firm</h1>
        <button className="btn btn-primary" onClick={() => { setShowForm(true); reset() }}>+ New Profile</button>
      </div>
      ...
      {status && (
        <div className="card mb-4">
          ...
          <div className="metric-grid">
            <div className="metric-card"><div className="metric-label">Phase</div><div className="metric-value">{status.current_phase}</div></div>
            <div className="metric-card"><div className="metric-label">Daily P&L</div><div className="metric-value" style={{ color: (status.daily_pnl_pct ?? 0) >= 0 ? 'var(--success)' : 'var(--danger)' }}>{(status.daily_pnl_pct ?? 0).toFixed(2)}%</div></div>
            <div className="metric-card"><div className="metric-label">Cumulative</div><div className="metric-value">${(status.cumulative_pnl ?? 0).toFixed(2)}</div></div>
            <div className="metric-card"><div className="metric-label">Trading Days</div><div className="metric-value">{status.trading_days}</div></div>
            <div className="metric-card"><div className="metric-label">Target Met</div><div className="metric-value" style={{ color: status.phase_target_met ? 'var(--success)' : 'var(--warn)' }}>{status.phase_target_met ? 'Yes' : 'No'}</div></div>
            <div className="metric-card"><div className="metric-label">Violation</div><div className="metric-value" style={{ color: status.violated ? 'var(--danger)' : 'var(--success)' }}>{status.violated ? status.violation_reason : 'None'}</div></div>
          </div>
        </div>
      )}
      ...
      {showForm && (
        <div className="card mb-4" style={{ maxWidth: 500 }}>
          ...
          <form onSubmit={handleSubmit(handleCreate)} style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            <div>
              <input className="input" placeholder="Profile ID" {...register('id')} />
            ...
            <div className="grid-2">
              <div>
                <label className="text-muted">Max Daily Loss %</label>
              ...
              <div>
                <label className="text-muted">Max Drawdown %</label>
            ...
            <div className="grid-2">
              <div>
                <label className="text-muted">Phase 1 Target %</label>
              ...
              <div>
                <label className="text-muted">Phase 2 Target %</label>
            ...
            <div className="grid-2">
              <div>
                <label className="text-muted">Max Positions</label>
              ...
              <div>
                <label className="text-muted">Min Trading Days</label>
      ...
      <div className="card">
        ...
        {profiles.length === 0 ? <p className="text-muted">No profiles</p> : (
          <table className="data-table">
            <thead><tr><th>ID</th><th>Name</th><th>Daily Loss</th><th>Max DD</th><th>Phase 1</th><th>Phase 2</th><th>Active</th><th>Actions</th></tr></thead>
            <tbody>
              {profiles.map(p => (
                <tr key={p.id}>
                  ...
                  <td>{activeId === p.id ? <span className="badge badge-ok">Active</span> : '—'}</td>
                  <td>
                    <div className="flex gap-1">
                      {activeId !== p.id && <button className="btn btn-outline" style={{ padding: '2px 8px', fontSize: 11 }} onClick={() => handleSetActive(p.id)}>Activate</button>}
                      <button className="btn btn-outline" style={{ padding: '2px 8px', fontSize: 11, color: 'var(--danger)' }} onClick={() => handleDelete(p.id)}>Delete</button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {confirmDelete && (
        <ConfirmDialog
          ...
          message="Delete this prop firm profile? This action cannot be undone."
          confirmLabel="Delete"
          danger
