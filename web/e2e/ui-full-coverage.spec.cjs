...
test.describe('Backtest Page', () => {
  ...
  test('Submit matrix backtest with all strategies', async ({ request }) => {
    const r = await request.post(`${BASE}/api/v1/backtests`, {
      data: {
  ...
  test('Backtest with invalid symbol returns warning', async ({ request }) => {
    const r = await request.post(`${BASE}/api/v1/backtests`, {
      data: {
    ...
    const d = await r.json();
    expect(d.status === 'completed' || d.warnings).toBeTruthy();
  });

  test('Backtest metrics endpoint', async ({ request }) => {
    const r = await request.get(`${BASE}/api/v1/backtests/bt-20260630001316-ma_crossover/metrics`);
    expect(r.status()).toBeLessThan(500);
  });
});
...
test.describe('Execution Page', () => {
  ...
  test('Cancel all orders', async ({ request }) => {
    const r = await request.delete(`${BASE}/api/v1/orders`);
    expect(r.status()).toBeLessThan(500);
  });
});

test.describe('Navigation & Routing', () => {
  test('All core API endpoints respond', async ({ request }) => {
    const endpoints = [
...
test.describe('Edge Cases & Error Handling', () => {
  ...
  test('Negative capital in backtest', async ({ request }) => {
    const r = await request.post(`${BASE}/api/v1/backtests`, {
      data: {
        strategy_ids: ['ma_crossover'],
        symbols: ['SPX500'],
